...
const dependenciesMet = ([wire1, op, wire2], values) => {
    ...
    if ((!isNaN(wire1) || values[wire1] !== undefined) &&
        (!isNaN(wire2) || values[wire2] !== undefined)) return true;
...
const part2 = (instructions, part1Result) => {
    ...
    const values = calculateValues(instructions, { b: part1Result ? part1Result : part1(input) });
    return values[finalWire];
};
